...
const Form = () => {
  ...
  const [decisionMaker, setDecisionMaker] = useState(null);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const decisionsDoc = await db.collection("decisions").doc(id).get();
      const decision = decisionsDoc.data();
      setDecision(decision);
      formik.setValues(decision);


      if (decision.decisionMaker !== "") {
        const res = await decision.decisionMaker.get();
        const decisionMaker = res.data();
        setDecisionMaker(decisionMaker);
      }
      if (decision.impacts !== "") {
        // const res = await decision.impacts.get();
        // const impact = res.data();
        setImpact(decision.impacts);
      }
      if (decision.priorities !== "") {
        // const res = await decision.priorities.get();
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      await db.collection("decisions").doc(id).set(values);
  ...
  return (
    <>
      {decision && (
        <div>
          <Header>Edit {decision.name} Decision</Header>
          <StyledForm onSubmit={formik.handleSubmit}>
            ...
            <StyledInputWrapper error={!!errors.priority}>
              <StyledSelect
                ...
                name="priority"
              >
                {priority && <option value={priority} label={priority} />}
            ...
            <StyledInputWrapper error={!!errors.impact}>
              <StyledSelect
                ...
                name="impact"
              >
                {impact && <option value={impact} label={impact} />}
            ...
            <StyledInputWrapper error={!!errors.decisionMaker}>
              ...
              >
                {decisionMaker && (
                  <option value={decisionMaker.id} label={decisionMaker.name} />
                )}
